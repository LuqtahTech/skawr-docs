...
| Tier | Price | Products | Searches/mo | Support | Key Features |
|------|-------|----------|-------------|---------|--------------|
| Growth | $99/mo | 50,000 | 500,000 | Email | Search + funnel analytics, AI reranking, personalization |
| Pro | $199/mo | 50,000 | 500,000 | WhatsApp | Everything in Growth + Skawr Bar, search widget, SkawrBot (static Q&A assistant) |
| Scale | $399/mo | 500,000 | 5,000,000 | WhatsApp | Everything in Pro + heatmaps, attribution (via analytics SDK) |
| Enterprise | Custom | Custom | Custom | Dedicated | Contact sales |

**Key constraints:**
- No free tier, no free trial — users must pay (subscribe via Polar) before getting API access
- No Starter tier — minimum paid tier is Growth ($99/mo)
- Annual billing: 2 months free → Growth $990/yr, Pro $1,990/yr, Scale $3,990/yr
- Overage: Growth $0.50/1K, Pro $0.40/1K, Scale $0.30/1K extra products
- Do NOT add tier recommendation badges ("Most Popular", etc.)
- CRO is offered as an upsell for Pro and Scale clients
...
2. **No free tier, no free trial** — API access is blocked (402) until Polar activates the subscription after checkout
...
5. On payment success: Polar fires webhook → backend activates `APIClient` (`subscription_status=active`, correct tier)

### Access Model
- No free trial — API access is blocked (402 from QuotaEnforcementMiddleware) until subscription is active
- Signup creates an account and API keys but access is gated until checkout completes
