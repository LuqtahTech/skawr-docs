...
| Analytics | summary, top-events, recent-events, timeseries, funnel, funnel-trend, retention, cohort, paths, revenue, attribution, user/{user_id} |
| Saved funnels | `/api/v1/funnels` CRUD + `{id}/export-cohort` (converters/non-converters → cohort) |
| Insights | hero, feed, trends, pin, dismiss |
| Cohorts | CRUD, preview, refresh, sample users |
| Dashboards | CRUD, widgets, bulk layout save |
| Alerts | rule CRUD (`threshold`/`anomaly`/`no_data`/`funnel`), history, resolve |
| Projects + Members | project CRUD, API keys, team members (invite/role/remove/leave/transfer); Shared Projects via `resolve_project_access` |
...
- **Funnels** (`GET /api/v1/analytics/funnel`) walk each identity in true time order.
  ...
  `window_hours` optionally bounds the first-to-last journey; `mode` is `ordered`
  (default) or `unordered` (any-order within the window); `exclude_between` names
  events that disqualify a user (funnel-wide). The response carries
  `median_time_to_convert_s` plus a `time_to_convert` distribution (p25/p50/p75/
  p90). Each step also carries `revenue_sar` (SAR the cohort reaching that step
  generated, so the gap between steps is revenue lost to that drop-off; null when
  the project has no revenue events) and `top_page` (the step's most common page,
  used for the heatmap "why" drilldown).
- **Funnel trend** (`GET /api/v1/analytics/funnel-trend`) recomputes a funnel's
  overall conversion for each entry cohort, bucketed by `day` or `week` (the
  funnels page "Conversion over time" chart).
- **Saved funnels** (`/api/v1/funnels`) are first-class named funnels modeled on
  cohorts (team-visible + `is_private`, migration `0020`); `POST
  /api/v1/funnels/{id}/export-cohort` materializes converters or non-converters
  into a cohort. The funnels page also has a node-edge "Flow view" and a what-if
  simulator, both computed from the funnel response (no extra endpoint).
...
| `event_definitions` | Display-name layer: friendly bilingual labels + optional count units per raw event key (`event_name`, `display_name_en/ar`, `description_en/ar`, `unit_en/ar`, `verified`); unique on (project, event_name). Migrations `0016`, `0017` |
| `project_members` | Team access (Shared Projects): (`project_id`, `user_id`, `role` owner/admin/editor/viewer). Analytics-owned; authorization funnels through `resolve_project_access`. Migration `0018` |
| `saved_funnels` | First-class named funnels modeled on cohorts (`name`, `is_private`, `definition` JSONB with steps/window/mode); team-visible with a private flag. Migration `0020` (`cohorts.is_private` added in `0019`) |
| `alert_rules` + `alert_history` | Alert rule definitions (`alert_type` threshold/anomaly/no_data/funnel, `condition` JSONB, `channels`, cooldown) and their fired/resolved history |
...
### Auto-Capture Events
- `$pageview` (on by default)
- `$click` (opt-in)
- `$form_submit` (opt-in)
- `$error` / `$unhandled_rejection` (opt-in)

As of `@skawr/analytics-react` **0.7.0**, click/form/error autocapture is opt-in
(default off) for privacy; only pageviews are on by default. Enable the streams
you want via `autoClicks` / `autoForms` / `autoErrors`. The published 0.6.0
defaulted them ON, so apps relying on 0.6.0 defaults must pin these flags before
upgrading (skawr-web does). Every event also carries an `sdk_source` tag; the
first-party SDKs set it to their platform (`react`, `web`, `node`, `python`,
`flutter`), distinct from the tool tags (`skawrbar`, `skawrbot`, `search-widget`,
`survey`, `batch-api`). The `$identify` sentinel is tagged too.
...
| Route | Purpose |
|-------|---------|
| `/` | Home: a lead KPI band (events/users/sessions, 7d, with deltas), then the insight feed (hero card + scannable mini-insights with colored category rails, grouped into "Needs attention" vs "Positive") with pin/dismiss, then the top-events + recent-events + by-tool LiveDataPanel |
| `/events` | Lead KPI band (total events, users, event types, top event) over a filterable live event stream |
| `/funnels` | Funnel suite: bolder step bars (drop shown as a muted remainder, biggest-drop emphasized, per-step revenue + revenue-lost), OR/branching + Advanced (unordered, exclusion) steps, conversion window, time-to-convert distribution, saved funnels + export-to-cohort, a node-edge "Flow view", a conversion-over-time chart, a what-if simulator, and a per-step "See heatmap" drilldown |
...
| `/behavior` | Monitor-first: lead KPI band (frustration clicks, quick-back pages, JS errors, affected users) + a "most frustrating page" callout (impact-normalized score), then collapsible detail sections/sub-sections for rage/dead clicks, quick backs, JS errors, and most-clicked elements |
| `/attribution` | Lead KPI band + "biggest revenue driver" callout, then multi-touch attribution: conversion-event selector, five models, lookback, SAR, `(direct)` reconciliation, and ROAS |
| `/revenue` | Lead KPI band (revenue, transactions, paying users, AOV) + "biggest revenue driver" callout, then revenue in SAR by day/event/user, unconvertible currencies surfaced separately |
...
| `/journeys` | Lead KPI band (users started, distinct paths, top-path share, avg length) + "most common path" callout, then the React Flow path graph |
| `/dashboards` | 12-column grid dashboards (KPI/line/funnel/cohort/retention/text widgets) |
| `/settings` | Project settings + API key CRUD; `/settings/members` (team access), `/settings/privacy` (consent + retention) |
| `/settings/alerts` | Alert rules (threshold, anomaly, no-data, funnel-conversion) with email/webhook delivery and a fired/resolved history |
...
Event rules, Data dictionary, Dashboards. Under the Workspace group: Alerts, Settings.

**Monitor-first is the UI standard (Phase A2).** Every screen opens with a lead
KPI band or one lead visual and surfaces the biggest thing first, then puts raw
tables behind progressive disclosure (collapsible sections). Do not ship a
screen that opens on an unranked table. The topbar "Live" chip reflects real
data freshness (green LIVE if an event arrived in the last hour, amber IDLE, else
NO DATA). See `skawr-analytics/docs/features-catalog.md` for the full
tab-by-tab feature list and `skawr-analytics/docs/funnels-improvement-scoping.md`
for the analytics UX roadmap.

Readability: event names render as friendly labels everywhere via the data dictionary
(raw keys only in developer surfaces), including the home LiveDataPanel. Plain-language
captions sit above the retention, funnel, and revenue charts. The home insight feed and
hero are bilingual with a recommended-action block per card.
...
- **Tier gating matches the pricing page** (this corrects an earlier "gating removed" note): analytics is bundled with the Search SaaS (no separate analytics billing), but not everything is on every tier. Core product analytics (events, funnels, retention, cohorts, journeys, revenue, surveys, insights) is on all paid tiers; **Heatmaps and Revenue Attribution are Scale-tier**; the onboarding widgets (Skawr Bar, Search Widget, SkawrBot, NPS Surveys) are **Pro and Scale** only. Gates use `RequireFullTier` / entitlement-follows-owner.
...
- **Funnels are true sequential walks with OR-groups**: step i must occur after the occurrence that satisfied step i-1. A step can match any of several events via pipe syntax (`add_to_cart|buy_now`), an optional `window_hours` bounds first-to-last, and `median_time_to_convert_s` (plus a p25/p50/p75/p90 `time_to_convert` distribution) is reported for full converters. `mode=unordered` counts any-order completion; `exclude_between` disqualifies users funnel-wide. Each step also returns `revenue_sar` (cohort revenue at that step, so step-to-step gaps are revenue lost) and `top_page` (for the heatmap "why" drilldown). Funnels can be saved, charted over time, viewed as a node-edge flow, simulated (what-if), and alerted on (a `funnel` alert type fires when overall conversion crosses a percent threshold).
...
- **Display-name layer (data dictionary)**: raw event keys are the immutable engineering contract and are never rewritten. Each key can get an editable, bilingual (EN/AR) display label + description (`event_definitions` table, `/api/v1/event-definitions`, `/data-dictionary` page). Friendly labels render everywhere a person reads data (events, funnels, journeys, revenue, insights); raw keys stay only in developer surfaces (hover title, deep-link params). Unlabeled keys fall back to a de-slugged form (`add_to_cart` becomes "Add to Cart"), computed identically on backend and frontend. Each key can also carry an optional bilingual count unit (`unit_en` plural, `unit_ar` singular-after-number) so insight copy reads "843 product views this week"; when unset, copy falls back to a grammar-free label-lead frame ("Product Viewed: 843 this week").
